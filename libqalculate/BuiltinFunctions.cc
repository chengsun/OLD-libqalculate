...
BitAndFunction::BitAndFunction() : MathFunction("bitand", 1) {
	VectorArgument *arg = new VectorArgument("");
	arg->addArgument(new IntegerArgument("", ARGUMENT_MIN_MAX_NONE, true, true));
	setArgumentDefinition(1, arg);
}
int BitAndFunction::calculate(MathStructure &mstruct, const MathStructure &vargs, const EvaluationOptions &eo) {
	if(vargs[0].size() > 0) {
		Number nr = vargs[0][0].number();;
		for(size_t i = 1; i < vargs[0].size(); i++) {
			nr.bitAnd(vargs[0][i].number());
		}
		mstruct = nr;
	}
	return 1;
}
BitOrFunction::BitOrFunction() : MathFunction("bitor", 1) {
	VectorArgument *arg = new VectorArgument("");
	arg->addArgument(new IntegerArgument("", ARGUMENT_MIN_MAX_NONE, true, true));
	setArgumentDefinition(1, arg);
}
int BitOrFunction::calculate(MathStructure &mstruct, const MathStructure &vargs, const EvaluationOptions &eo) {
	if(vargs[0].size() > 0) {
		Number nr = vargs[0][0].number();;
		for(size_t i = 1; i < vargs[0].size(); i++) {
			nr.bitOr(vargs[0][i].number());
		}
		mstruct = nr;
	}
	return 1;
}
BitXorFunction::BitXorFunction() : MathFunction("bitxor", 1) {
	VectorArgument *arg = new VectorArgument("");
	arg->addArgument(new IntegerArgument("", ARGUMENT_MIN_MAX_NONE, true, true));
	setArgumentDefinition(1, arg);
}
int BitXorFunction::calculate(MathStructure &mstruct, const MathStructure &vargs, const EvaluationOptions &eo) {
	if(vargs[0].size() > 0) {
		Number nr = vargs[0][0].number();;
		for(size_t i = 1; i < vargs[0].size(); i++) {
			nr.bitXor(vargs[0][i].number());
		}
		mstruct = nr;
	}
	return 1;
}
BitEqvFunction::BitEqvFunction() : MathFunction("biteqv", 1) {
	VectorArgument *arg = new VectorArgument("");
	arg->addArgument(new IntegerArgument("", ARGUMENT_MIN_MAX_NONE, true, true));
	setArgumentDefinition(1, arg);
}
int BitEqvFunction::calculate(MathStructure &mstruct, const MathStructure &vargs, const EvaluationOptions &eo) {
	if(vargs[0].size() > 0) {
		Number nr = vargs[0][0].number();;
		for(size_t i = 1; i < vargs[0].size(); i++) {
			nr.bitEqv(vargs[0][i].number());
		}
		mstruct = nr;
	}
	return 1;
}
BitNotFunction::BitNotFunction() : MathFunction("bitnot", 1) {
	setArgumentDefinition(1, new IntegerArgument("", ARGUMENT_MIN_MAX_NONE, true, true));
}
int BitNotFunction::calculate(MathStructure &mstruct, const MathStructure &vargs, const EvaluationOptions &eo) {
	FR_FUNCTION(bitNot)
}
