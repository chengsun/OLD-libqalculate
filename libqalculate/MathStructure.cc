...
int MathStructure::merge_addition(MathStructure &mstruct, const EvaluationOptions &eo, MathStructure *mparent, size_t index_this, size_t index_mstruct) {
	...
	if(isZero()) {
		if(mparent) {
			mparent->swapChilds(index_this + 1, index_mstruct + 1);
		} else {
...
bool addablePower(const MathStructure &mstruct, const EvaluationOptions &eo) {
	if(mstruct[0].representsNonNegative()) return true;
	if(mstruct[1].representsInteger()) return true;
	return eo.allow_complex && mstruct[0].representsNegative() && mstruct[1].isNumber() && mstruct[1].number().isRational() && mstruct[1].number().denominatorIsEven();
}

int MathStructure::merge_multiplication(MathStructure &mstruct, const EvaluationOptions &eo, MathStructure *mparent, size_t index_this, size_t index_mstruct, bool do_append) {
	if(mstruct.type() == STRUCT_NUMBER && m_type == STRUCT_NUMBER) {
		Number nr(o_number);
	...
	if(eo.reduce_divisions) {
		if(!isNumber() && mstruct.isPower() && mstruct[0].isAddition() && mstruct[0].size() > 1 && mstruct[1].isNumber() && mstruct[1].number().isMinusOne()) {
			if((!isPower() || !CHILD(1).hasNegativeSign()) && representsNumber() && mstruct[0].representsNumber() && ((eo.assume_denominators_nonzero && !mstruct[0].representsZero(true)) || mstruct[0].representsNonZero(true))) {
				mnum = this;
				mden = &mstruct[0];
			}
		} else if(!mstruct.isNumber() && isPower() && CHILD(0).isAddition() && CHILD(0).size() > 1 && CHILD(1).isNumber() && CHILD(1).number().isMinusOne()) {
			if((!mstruct.isPower() || !mstruct[1].hasNegativeSign()) && mstruct.representsNumber() && CHILD(0).representsNumber() && ((eo.assume_denominators_nonzero && !CHILD(0).representsZero(true)) || CHILD(0).representsNonZero(true))) {
				mnum = &mstruct;
				mden = &CHILD(0);
			}
		}
				...
				case STRUCT_POWER: {
					...
					if(mstruct[1].hasNegativeSign()) {
						...
						for(size_t i = 0; i < SIZE; i++) {
							...
							else ret = CHILD(i).merge_multiplication(*mstruct2, eo, NULL, 0, 0, false);
							if(ret == 0) {
								ret = mstruct2->merge_multiplication(CHILD(i), eo, NULL, 0, 0, false);
								if(ret >= 1) {
									mstruct2->ref();
				...
				case STRUCT_POWER: {
					if(mstruct[1].isNumber()) {
						if(*this == mstruct[0] && addablePower(mstruct, eo)) {
							...
							for(size_t i = 0; i < SIZE; i++) {
								int ret = CHILD(i).merge_multiplication(mstruct, eo, NULL, 0, 0, false);
								if(ret == 0) {
									ret = mstruct.merge_multiplication(CHILD(i), eo, NULL, 0, 0, false);
									if(ret >= 1) {
										if(ret == 2) ret = 3;
										else if(ret == 3) ret = 2;
										mstruct.ref();
			...
			switch(mstruct.type()) {
				...
				case STRUCT_POWER: {
					if(mstruct[0] == CHILD(0)) {
						bool b = CHILD(0).representsNonNegative();
						if(!b) {
							b = CHILD(1).representsInteger()  && mstruct[1].representsInteger();
						}
						if(!b) {
							b = eo.allow_complex && CHILD(0).representsNegative();
							if(b) b = (CHILD(1).representsInteger() || (CHILD(1).isNumber() && CHILD(1).number().isRational() && CHILD(1).number().denominatorIsEven()));
							if(b) b = (mstruct[1].representsInteger() || (mstruct[1].isNumber() && mstruct[1].number().isRational() && mstruct[1].number().denominatorIsEven()));
						}
						if(b) {
							b = (eo.assume_denominators_nonzero && !CHILD(0).representsZero(true)) || CHILD(0).representsNonZero(true) || (CHILD(1).representsPositive() && mstruct[1].representsPositive()) || (CHILD(1).representsNegative() && mstruct[1].representsNegative());
						}
						if(b) {
					...
					} else if(mstruct[1] == CHILD(1) && (mstruct[1].representsInteger() || CHILD(0).representsPositive() || mstruct[0].representsPositive())) {
				...
				default: {
					if(!mstruct.isNumber() && CHILD(1).isNumber() && CHILD(0) == mstruct && addablePower(*this, eo)) {
						if((eo.assume_denominators_nonzero && !CHILD(0).representsZero(true)) || CHILD(0).representsNonZero(true) || CHILD(1).representsPositive()) {
							CHILD(1).number()++;
							MERGE_APPROX_AND_PREC(mstruct)
							calculateRaiseExponent(eo, mparent, index_this);
							return 1;
		...
		default: {
			switch(mstruct.type()) {
				...
				default: {
					...
					if(isZero() && !mstruct.representsUndefined(true, true, !eo.assume_denominators_nonzero) && (!eo.keep_zero_units || !mstruct.containsRepresentativeOfType(STRUCT_UNIT, true, true))) {
						MERGE_APPROX_AND_PREC(mstruct)
						return 2;
					}
					if(equals(mstruct)) {
...
int MathStructure::merge_power(MathStructure &mstruct, const EvaluationOptions &eo, MathStructure *mparent, size_t index_this, size_t index_mstruct) {
	if(mstruct.type() == STRUCT_NUMBER && m_type == STRUCT_NUMBER) {
		Number nr(o_number);
	...
	switch(m_type) {
		...
		case STRUCT_POWER: {
			if(mstruct.representsInteger() || CHILD(0).representsNonNegative()) {
				if(CHILD(1).isNumber() && CHILD(1).number().isRational() && (CHILD(1).isInteger() || CHILD(1).number().denominatorIsEven() || (!CHILD(1).number().numeratorIsEven() && mstruct.representsOdd()) || CHILD(0).representsNonNegative()) && (eo.allow_complex || !CHILD(1).number().denominatorIsEven() || CHILD(0).representsPositive() || CHILD(0).representsComplex())) {
					...
					calculateRaiseExponent(eo, mparent, index_this);					
					return 1;
				} else if(CHILD(0).representsNonNegative() || CHILD(1).representsInteger()) {
					mstruct.ref();
					MERGE_APPROX_AND_PREC(mstruct)
					CHILD(1).multiply_nocopy(&mstruct, true);
					CHILD(1).calculateMultiplyLast(eo, true, this, 1);
		...
		default: {
			...
			if(mstruct.isAddition()) {
				bool b = representsNonNegative();
				if(!b) {
					b = true;
					bool bneg = representsNegative();
					for(size_t i = 0; i < mstruct.size(); i++) {
				...
				if(b) {
					...
					for(size_t i = 0; i < mstruct.size(); i++) {
						if(isZero() && !mstruct[i].representsPositive()) continue;
...
int MathStructure::merge_logical_and(MathStructure &mstruct, const EvaluationOptions &eo, MathStructure *mparent, size_t index_this, size_t index_mstruct) {

	if(mstruct.isComparison() && mstruct.comparisonType() == COMPARISON_GREATER && mstruct[1].isZero()) {
		mstruct.setToChild(1, true, mparent, index_mstruct);
		return merge_logical_and((*mparent)[index_mstruct], eo, mparent, index_this, index_mstruct);
	}
	if(isComparison() && comparisonType() == COMPARISON_GREATER && CHILD(1).isZero()) {
		setToChild(1, true, mparent, index_this);
		return (*mparent)[index_this].merge_logical_and(mstruct, eo, mparent, index_this, index_mstruct);
	}
				...
				case COMPARISON_RESULT_LESS: {
					switch(ct1) {
						...
						case COMPARISON_NOT_EQUALS: {
							switch(ct2) {
								case COMPARISON_EQUALS_GREATER: {}
								case COMPARISON_GREATER: {set_nocopy(mstruct, true); return 3;}
								default: {}
							}
							break;
						}
...
int MathStructure::merge_logical_or(MathStructure &mstruct, const EvaluationOptions &eo, MathStructure *mparent, size_t index_this, size_t index_mstruct) {

	if(mstruct.isComparison() && mstruct.comparisonType() == COMPARISON_GREATER && mstruct[1].isZero()) {
		mstruct.setToChild(1, true, mparent, index_mstruct);
		return merge_logical_or((*mparent)[index_mstruct], eo, mparent, index_this, index_mstruct);
	}
	if(isComparison() && comparisonType() == COMPARISON_GREATER && CHILD(1).isZero()) {
		setToChild(1, true, mparent, index_this);
		return (*mparent)[index_this].merge_logical_or(mstruct, eo, mparent, index_this, index_mstruct);
	}
			...
			switch(cr) {
				...
				case COMPARISON_RESULT_EQUAL: {
					...
					switch(ct_comp) {
						case COMPARISON_EQUALS: {
							switch(ct2) {
								case COMPARISON_NOT_EQUALS: {clear(true); o_number.setTrue(); MERGE_APPROX_AND_PREC(mstruct) return 1;}
								case COMPARISON_EQUALS_LESS: {}
						...
						case COMPARISON_NOT_EQUALS: {
							switch(ct2) {
								case COMPARISON_EQUALS_LESS: {}
								case COMPARISON_EQUALS_GREATER: {}
								case COMPARISON_EQUALS: {clear(true); o_number.setTrue(); MERGE_APPROX_AND_PREC(mstruct) return 1;}								
								case COMPARISON_LESS: {}
						...
						case COMPARISON_EQUALS_LESS: {
							switch(ct2) {
								case COMPARISON_NOT_EQUALS: {}
								case COMPARISON_GREATER: {}
								case COMPARISON_EQUALS_GREATER: {clear(true); o_number.setTrue(); MERGE_APPROX_AND_PREC(mstruct) return 1;}
								case COMPARISON_EQUALS: {}
								case COMPARISON_LESS: {MERGE_APPROX_AND_PREC(mstruct) return 2;}
								default: {}
							}
						...
						case COMPARISON_EQUALS_GREATER: {
							switch(ct2) {
								case COMPARISON_NOT_EQUALS: {}
								case COMPARISON_LESS: {}
								case COMPARISON_EQUALS_LESS: {clear(true); o_number.setTrue(); MERGE_APPROX_AND_PREC(mstruct) return 1;}
								case COMPARISON_EQUALS: {}
								case COMPARISON_GREATER: {MERGE_APPROX_AND_PREC(mstruct) return 2;}
								default: {}
							}
					...
					switch(ct1) {
						case COMPARISON_EQUALS: {
							switch(ct2) {
								case COMPARISON_NOT_EQUALS: {}
								case COMPARISON_EQUALS_LESS: {}
								case COMPARISON_LESS: {set_nocopy(mstruct, true); return 3;}
								default: {}
							}
							break;
						}
						case COMPARISON_NOT_EQUALS: {
							switch(ct2) {
								...
								case COMPARISON_GREATER: {MERGE_APPROX_AND_PREC(mstruct) return 2;}
								case COMPARISON_EQUALS_LESS: {}
								case COMPARISON_LESS: {clear(true); o_number.setTrue(); MERGE_APPROX_AND_PREC(mstruct) return 1;}
								default: {}
							}
							break;
						}
...
int MathStructure::merge_logical_xor(MathStructure &mstruct, const EvaluationOptions&, MathStructure *mparent, size_t index_this, size_t index_mstruct) {
...
int MathStructure::merge_bitwise_and(MathStructure &mstruct, const EvaluationOptions &eo, MathStructure*, size_t, size_t) {
	if(mstruct.type() == STRUCT_NUMBER && m_type == STRUCT_NUMBER) {
		Number nr(o_number);
...
int MathStructure::merge_bitwise_or(MathStructure &mstruct, const EvaluationOptions &eo, MathStructure*, size_t, size_t) {
	if(mstruct.type() == STRUCT_NUMBER && m_type == STRUCT_NUMBER) {
		Number nr(o_number);
...
int MathStructure::merge_bitwise_xor(MathStructure &mstruct, const EvaluationOptions &eo, MathStructure*, size_t, size_t) {
	if(mstruct.type() == STRUCT_NUMBER && m_type == STRUCT_NUMBER) {
		Number nr(o_number);
...
#define MERGE_ALL(FUNC, TRY_LABEL) 	if(recursive) {\
					...
					for(size_t i = 0; i < SIZE - 1; i++) {\
						...
						for(; i2 < i; i2++) {\
							...
							if(r == 0) {\
								SWAP_CHILDS(i2, i);\
								r = CHILD(i2).FUNC(CHILD(i), eo, this, i2, i);\
								if(r < 1) {\
									SWAP_CHILDS(i2, i);\
								}\
							}\
						...
						for(i2 = i + 1; i2 < SIZE; i2++) {\
							...
							if(r == 0) {\
								SWAP_CHILDS(i, i2);\
								r = CHILD(i).FUNC(CHILD(i2), eo, this, i, i2);\
								if(r < 1) {\
									SWAP_CHILDS(i, i2);\
								} else if(r == 2) {\
									r = 3;\
...
#define MERGE_INDEX(FUNC, TRY_LABEL)	bool b = false;\
					...
					for(size_t i = 0; i < index; i++) {\
						...
						if(r == 0) {\
							SWAP_CHILDS(i, index);\
							r = CHILD(i).FUNC(CHILD(index), eo, this, i, index);\
							if(r < 1) {\
								SWAP_CHILDS(i, index);\
							} else if(r == 2) {\
								r = 3;\
					...
					for(size_t i = index + 1; i < SIZE; i++) {\
						...
						if(r == 0) {\
							SWAP_CHILDS(index, i);\
							r = CHILD(index).FUNC(CHILD(i), eo, this, index, i);\
							if(r < 1) {\
								SWAP_CHILDS(index, i);\
							} else if(r == 2) {\
								r = 3;\
...
bool MathStructure::calculateNegate(const EvaluationOptions &eo, MathStructure *mparent, size_t index_this) {
	return calculateMultiply(m_minus_one, eo, mparent, index_this);
}
...
bool MathStructure::calculateMultiplyIndex(size_t index, const EvaluationOptions &eo, bool check_size, MathStructure *mparent, size_t index_this) {
	...
	MERGE_INDEX(merge_multiplication, try_multiply_index)
	MERGE_INDEX2
...
void try_isolate_x(MathStructure &mstruct, EvaluationOptions &eo3, const EvaluationOptions &eo) {
	if(mstruct.isComparison()) {
		if(mstruct.comparisonType() == COMPARISON_EQUALS || mstruct.comparisonType() == COMPARISON_NOT_EQUALS) {
			...
			if(mtest.isolate_x(eo3, m_undefined, true)) {
				mstruct = mtest;
			}
		}
	} else {
...
bool compare_delete(MathStructure &mnum, MathStructure &mden, bool &erase1, bool &erase2, const EvaluationOptions &eo) {
	...
	erase2 = false;
	if(mnum == mden) {
		if(!(eo.assume_denominators_nonzero && !mnum.representsZero(true)) && !mnum.representsNonZero(true)) {
			if(mnum.isPower()) {
				mnum.setToChild(1);
				mden.setToChild(1);
				return true;
	...
	if(mbase1->equals(*mbase2)) {
		if(mexp1 && mexp2) {
			if(mexp1->number().isLessThan(mexp2->number())) {
				...
				if(!(eo.assume_denominators_nonzero && !mbase2->representsZero(true)) && !mbase2->representsNonZero(true)) {
			...
			if(!(eo.assume_denominators_nonzero && !mbase2->representsZero(true)) && !mbase2->representsNonZero(true)) return false;
			mexp1->number()--;
			erase2 = true;
			return true;
		} else if(mexp2) {
...
MathStructure &MathStructure::eval(const EvaluationOptions &eo) {
	...
	if(eo2.isolate_x && !eo2.assume_denominators_nonzero && containsType(STRUCT_COMPARISON)) {		
		EvaluationOptions eo3 = eo2;
		eo3.assume_denominators_nonzero = true;
		try_isolate_x(*this, eo3, eo2);
	}
					...
					for(size_t i = 0; i < SIZE; i++) {
						if(!CHILD(i).isAddition()) {
							...
							if(ret == 0) {
								...
								if(ret > 0) {
									factor_mstruct->ref();
									setChild_nocopy(factor_mstruct, i + 1);
								}
							}
							if(ret > 0) b = true;
							break;
...
void MathStructure::swapChilds(size_t index1, size_t index2) {
	if(index1 > 0 && index2 > 0 && index1 <= SIZE && index2 <= SIZE) {
		SWAP_CHILDS(index1 - 1, index2 - 1)
	}
}
...
bool test_comparisons(const MathStructure &msave, MathStructure &mthis, const MathStructure &x_var, const EvaluationOptions &eo, bool sub) {
	if(mthis.isComparison() && mthis[0] == x_var) {
		...
		if(mtest.isComparison()) {
			...
			CALCULATOR->beginTemporaryStopMessages();
			mtest.eval(eo2);
			if(CALCULATOR->endTemporaryStopMessages() == 0 && mtest.isNumber() && ((mthis.comparisonType() == COMPARISON_EQUALS && mtest.number().getBoolean() > 0) || (mthis.comparisonType() == COMPARISON_NOT_EQUALS && mtest.number().getBoolean() < 1))) {
				return true;
			}
		}
		if(!sub) mthis = msave;
...
bool MathStructure::isolate_x(const EvaluationOptions &eo, const MathStructure &x_varp, bool check_result) {
	...
	if(x_varp.isUndefined()) {
		...
		if(!check_result || (ct_comp != COMPARISON_EQUALS && ct_comp != COMPARISON_NOT_EQUALS)) {
	...
	switch(CHILD(0).type()) {
		case STRUCT_ADDITION: {
			...
			if(!CHILD(1).representsZero(true)) {
				int index1 = -1;
				int index2 = -1;
				for(size_t i2 = 0; i2 < CHILD(0).size(); i2++) {
					if(index1 < 0 && CHILD(0)[i2].isMultiplication()) {
						for(size_t i = 0; i < CHILD(0)[i2].size(); i++) {
							if(CHILD(0)[i2][i].isPower() && CHILD(0)[i2][i][1].isNumber() && CHILD(0)[i2][i][1].number().isMinusOne() && CHILD(0)[i2][i][0].contains(x_var) && !CHILD(0)[i2][i][0].representsZero(true)) {
								index1 = i2;
								index2 = i;
								break;
							}
						}
					} else if(CHILD(0)[i2].representsZero(true)) {
						index1 = -1;
						break;
					}
				}
		...
		case STRUCT_MULTIPLICATION: {
			...
			for(size_t i = 0; i < CHILD(0).size(); i++) {
				if(!CHILD(0)[i].contains(x_var)) {
					if(!CHILD(0)[i].representsZero(true)) {
						bool b2 = false;
						if(ct_comp != COMPARISON_EQUALS && ct_comp != COMPARISON_NOT_EQUALS) {
			...
			if(b) {
				...
				if(ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS) {
					vector<size_t> canzero;
					vector<size_t> cannotzero;
					for(size_t i = 0; i < CHILD(0).size(); i++) {
						if(CHILD(0)[i].contains(x_var) && (!CHILD(0)[i].isPower() || !CHILD(0)[i][1].representsNegative())) {
							canzero.push_back(i);	
						} else {
							cannotzero.push_back(i);	
						}	
					}
					if(canzero.size() == 1) {
						CHILD(0).setToChild(canzero[0] + 1);	
						return true;
					} else if(canzero.size() > 1) {
						...
						for(size_t i = 0; i < canzero.size(); i++) {
							CHILD(canzero[i]).transform(STRUCT_COMPARISON, m_zero);
							CHILD(canzero[i]).setComparisonType(ct);
							CHILD(canzero[i]).isolate_x(eo, x_var);
						}
						for(size_t i = 0; i < cannotzero.size(); i++) {
							ERASE(cannotzero[i])
						}
						calculatesub(eo2, eo2, false);
						return true;
					...
					if(ct_comp == COMPARISON_EQUALS_LESS || ct_comp == COMPARISON_EQUALS_GREATER) {
						mless1.setComparisonType(COMPARISON_EQUALS_LESS);
					} else {
						mless1.setComparisonType(COMPARISON_LESS);
					}
					mless1.isolate_x(eo, x_var);
					...
					if(ct_comp == COMPARISON_LESS || ct_comp == COMPARISON_EQUALS_LESS) {
						...
						CHILD(1).calculatesub(eo2, eo2, false);
						CHILD_UPDATED(1)
						calculatesub(eo2, eo2, false);
					} else {
						set(mless1);
		...
		case STRUCT_POWER: {
			if(CHILD(0)[0].contains(x_var)) {
				if(!CHILD(0)[1].contains(x_var)) {
					if(!CHILD(0)[1].representsNonNegative() && !CHILD(1).representsNonZero(true)) {
						if(ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS) {
							if(CHILD(1).representsZero(true)) return false;
						} else if(CHILD(1).representsZero(true)) {
					...
					bool pow_even = CHILD(0)[1].representsEven();
					if(!pow_even && !CHILD(0)[1].representsOdd()) return false;
					MathStructure exp(1, 1);
		...
		case STRUCT_FUNCTION: {
			if(CHILD(0).function() == CALCULATOR->f_ln && CHILD(0).size() == 1) {
				...
				if(CHILD(0)[0].contains(x_var)) {
					...
					CHILD(0).isolate_x(eo, x_var);
					CHILD(1).isolate_x(eo, x_var);
					CHILDREN_UPDATED
					calculatesub(eo2, eo2, false);
...
bool MathStructure::isRationalPolynomial() const {
	switch(m_type) {
		...
		case STRUCT_POWER: {
			return CHILD(1).isInteger() && CHILD(1).number().isNonNegative() && !CHILD(0).isMultiplication() && !CHILD(0).isAddition() && CHILD(0).isRationalPolynomial();
		}
		case STRUCT_UNIT: {}
		case STRUCT_FUNCTION: {}
		case STRUCT_VARIABLE: {}
		case STRUCT_SYMBOLIC: {
			return !representsUndefined(true, true);
		}
	}
	return false;
}
